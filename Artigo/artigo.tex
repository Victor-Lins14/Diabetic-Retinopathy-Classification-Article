...
camadas e o tratamento do desbalanceamento extremo são fatores determinantes
...
para triagem oftalmológica, especialmente em regiões com recursos
...
para melhorar o desempenho dos sistemas automatizados de apoio à
...
que bases de dados públicas frequentemente possuem predominância de
...
estratificada por classe. Para garantir a reprodutibilidade e a consistência das partições entre todas as arquiteturas avaliadas, foi definida uma \textit{seed} fixa igual a 42. Não foi utilizado um conjunto de teste externo independente, e a distribuição exata das amostras entre os conjuntos de treino e validação é detalhada na Tabela~\ref{tab:distribuicao_base}.
...
Observa-se forte desbalanceamento entre as classes, com predominância
...
treinamento, e não necessariamente à última época executada.
...
doença. Essa discrepância reforça a importância de avaliar
...
resultado geral acaba mascarado pela dominância da classe
...
A hipótese mais provável para esse resultado é a incompatibilidade entre as distribuições de probabilidade dos modelos: quando as saídas não estão calibradas numa mesma escala, a média ponderada tende a produzir predições inconsistentes. Modelos com tendência à superconfiança (\textit{overconfidence}) podem facilmente dominar a votação, anulando os acertos de redes mais conservadoras. Além disso, embora os pesos do ensemble tenham sido calculados com base no desempenho individual prévio, eles foram aplicados de forma empírica e estática, sem uma otimização sistemática (como a calibração de um meta-modelo em \textit{stacking}) sobre as predições de validação. Não se pode descartar também algum problema técnico ou inconsistência no carregamento e processamento dos \textit{checkpoints} de arquiteturas tão distintas durante a execução final. Assim, na configuração avaliada, o ensemble não apresentou ganho de desempenho suficiente para justificar o seu custo computacional adicional, visto que a execução simultânea de múltiplas arquiteturas profundas eleva drasticamente a latência de inferência, inviabilizando a sua aplicação ágil e escalável em ambientes clínicos reais de triagem.
...
Outra consideração técnica crítica levantada ao longo dos experimentos diz respeito à manipulação da resolução das imagens. Alterar as dimensões de entrada de um mesmo modelo pré-treinado em diferentes testes não é considerado uma boa prática computacional. Cada arquitetura é desenhada e otimizada matematicamente para trabalhar com dimensões específicas. Ainda que o \textit{framework} de Deep Learning readapte os tensores internamente por meio de \textit{adaptive pooling} para evitar erros de compilação, o número de parâmetros, o arranjo das camadas e a profundidade dos mapas de características permanecem inalterados. Forçar resoluções arbitrárias pode, consequentemente, comprometer a eficiência da extração de \textit{features} nativa da rede.

Por fim, a principal limitação metodológica deste estudo foi a ausência de um conjunto de teste isolado (\textit{test set} independente). Embora o uso de uma \textit{seed} fixa (42) tenha garantido que não houvesse mistura ou vazamento de dados de treino para validação, a arquitetura experimental pecou por utilizar o próprio subconjunto de validação tanto para acionar o \textit{early stopping} e escolher o melhor modelo, quanto para relatar as métricas finais do \textit{ensemble}. Essa dupla utilização da mesma partição introduz um viés otimista de avaliação (\textit{evaluation bias}), pois as configurações finais do sistema acabam por ser indiretamente sobreajustadas aos dados de validação, mascarando a real capacidade de generalização do algoritmo em dados totalmente inéditos.

Para trabalhos futuros, as prioridades absolutas incluem: o estabelecimento de um particionamento rigoroso dos dados em três vias (treino, validação e teste isolado); o respeito irrestrito às dimensões de entrada nativas documentadas para cada arquitetura; e a exploração de abordagens mais avançadas para mitigação do desbalanceamento extremo, como \textit{data augmentation} seletivo focado apenas nas classes minoritárias, o uso de \textit{Focal Loss} e calibração de limite de decisão (\textit{threshold moving}).
